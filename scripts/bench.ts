...
const mockItems = Array.from({ length: 10 }).map(
  (_, i) => `Mission Control Log Entry #${i + 100}`,
);
...
for (let i = 0; i < 1000; i++) {
  const frame = joinVertical(
    ...
    joinHorizontal(
      ...
      panel(
        pc.yellow('ACTIVE SUB-SYSTEMS\n') +
          mockItems
            .slice(0, 3)
            .map((it) => truncate(it, 40))
            .join('\n'),
      ),
    ),
  );
...
console.log(
  `Est. Throughput: ${pc.bold(Math.round(1000 / (total / 1000)))} frames/sec`,
);
