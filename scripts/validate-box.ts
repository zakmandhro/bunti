...
async function run() {
  ...
  assert(visibleWidth(fixed) === 20, "Fixed width (20) with border is exactly 20");
  
  const padded = bunti.box("HI", { padding: [1, 2], border: 'none' });
  const paddedLines = padded.split('\n');
  assert(visibleWidth(padded) === 6, "Padding [1,2] adds 4 cols to 'HI' (2+4=6)");
  assert(paddedLines.length === 3, "Padding [1,2] adds 2 rows to 1 line content (1+2=3)");

  const percent = bunti.box("HI", { width: "50%" }, 100);
  assert(visibleWidth(percent) === 50, "Percentage width ('50%') of 100 resolve to 50");
