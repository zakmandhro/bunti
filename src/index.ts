...
export const visibleWidth = (str: string) => stripAnsi(str).length;
...
export function box(content: string, options: StyleOptions = {}): string {
  ...
  // Top Border
  if (border) {
    out.push(color(border.tl + border.h.repeat(width) + border.tr));
  }

  // Top Padding
  for (let i = 0; i < paddingY; i++) {
    out.push((border ? color(border.v) : '') + ' '.repeat(width) + (border ? color(border.v) : ''));
  }
  ...
  for (const line of lines) {
    ...
    if (options.align === 'center') {
      pLeft = Math.floor(padTotal / 2);
      pRight = Math.ceil(padTotal / 2);
  ...
  // Bottom Border
  if (border) {
    out.push(color(border.bl + border.h.repeat(width) + border.br));
  }
...
export function joinHorizontal(...blocks: string[]): string {
  ...
  const blockWidths = parsedBlocks.map(block => 
    Math.max(...block.map(visibleWidth), 0)
  );
...
export function truncate(str: string, width: number, tail = '…'): string {
  ...
  return out + tail + '\x1b[0m'; // Reset at end
}
