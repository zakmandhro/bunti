...
export function Header(ctx: BuntiContext, props: HeaderProps) {
  ...
  box({
    ...
    const leftNode = engineBox(leftStr, { width: 12, align: 'left', border: 'none', padding: [0,0] });
    
    // 2. Center: Title
    const midStr = color.fg(fg, color.bold(props.title));
    const midNode = engineBox(midStr, { width: width - 28, align: 'center', border: 'none', padding: [0,0] });
    
    // 3. Right: Telemetry / Exit
    const rightStr = props.rightLabel ? color.dim(props.rightLabel) : '';
    const rightNode = engineBox(rightStr, { width: 12, align: 'right', border: 'none', padding: [0,0] });
