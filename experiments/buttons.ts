...
import { render, fg, visibleWidth, box as layoutBox } from '../src/index';

let clickCount = 0;
let lastMessage = "Awaiting interaction...";

render(({ wallpaper, width, height, color, state, blit, box, rect }) => {
  ...
  const drawButton = (label: string, x: number, y: number, clr: string, hover: string, active: string, onPress: () => void) => {
    const padding: [number, number] = [0, 2];
    const w = visibleWidth(label) + (padding[1] * 2) + 2;
    const h = 3; // 1 content line + 2 border lines

    const isHovered = state.mouseX >= x && state.mouseX < x + w && state.mouseY >= y && state.mouseY < y + h;
