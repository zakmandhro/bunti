...
bunti.render((ctx) => {
  ...
  const stationFlicker = stationOn || (stationProgress > 0 && ctx.flicker(0.7));
  ...
  ctx.box({ size: 60, border: 'none' }, (sub) => {
    logo.forEach((line) => {
      ...
      if (spaceFlicker) {
        sub.span({ color: spaceOn ? sub.color.cyan : sub.color.gray }, (s) => s.text(spacePart));
      } else {
        sub.text(' '.repeat(spacePart.length));
      }

      if (stationFlicker) {
        sub.span({ color: stationOn ? sub.color.magenta : sub.color.gray }, (s) => s.text(stationPart));
      } else {
