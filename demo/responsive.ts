...
bunti.render(({ wallpaper, box, color, width, height, blit, icon }) => {
  ...
  box({
    ...
    span({ color: color.bold }, ({ text }) => text(` ${icon('info')} BREAKOUT TEST\n\n`));
    text("This container implements SURGICAL WRAPPING. It automatically folds text at the boundary to prevent TTY artifacts. It also has a MIN-WIDTH of 30 chars, so it will refuse to collapse into nothingness.");
  });
  ...
  box({
    ...
    span({ color: color.bold }, ({ text }) => text(` ${icon('warning')} SIGNAL LOSS\n\n`));
    text("This container uses DEFAULT TRUNCATION. It will cut off the signal at the border, maintaining its rigid vertical height. This is the industrial standard for dense telemetry dashboards.");
  });
